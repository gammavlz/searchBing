...
class bing_api {
  ...
  public function __construct( $dork = "" , $number_pages = 10 , $count = 50, $sub_links = TRUE, $timeout = 30 )
  {
    self::$dork = $dork;
  ...
  private function filter_results( $source = "", $sub_links=TRUE )
  {
    ...
    foreach( $all_class as $class )
    {
      ...
      echo "<a href='".$results[$a]['main']['url'] = self::$links[] = $class[0]->getAttribute('href')."'>'".$results[$a]['main']['title']."'</a>";
      echo "<br>";
      echo $results[$a]['main']['description'];
      echo "<br>";echo "<br>";
      ...
      if ( $sub_links === TRUE )
      {
        for( $i=1; $i<count($class);$i++)
        {
          ...
          echo "<a href='".$results[$a]['sub']['url'] = self::$links[] = $class[$i]->getAttribute('href')."'>'".$results[$a]['sub']['description']."'</a>";
          echo "<br>";
        }
      }
      $a+=1;
...
$search->setDork($palabra);
$search->setCount(10);// number of results on each page
